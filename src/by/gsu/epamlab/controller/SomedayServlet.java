...
public class SomedayServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
  
    public SomedayServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  final User user = (User) request.getSession().getAttribute(Constant.USER);
	    try{
	      ...
	      List<Tasks> tasksList = tasksSource.getTasksByTasksSomeday(user);
	      System.out.println(tasksList);
	      request.setAttribute(Constant.TASKS, tasksList);
	      ServletUtilite.jump(Constant.SOMEDAY, request, response);
